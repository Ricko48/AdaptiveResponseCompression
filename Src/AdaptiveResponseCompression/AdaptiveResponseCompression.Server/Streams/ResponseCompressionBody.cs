...
using SendFileFallback = Microsoft.AspNetCore.Http.SendFileFallback;
...
internal sealed class ResponseCompressionBody : Stream, IHttpResponseBodyFeature, IHttpsCompressionFeature
{
    ...
    private IAdaptiveCompressionProvider? _compressionProvider;
    ...
    internal ResponseCompressionBody(HttpContext context, IAdaptiveResponseCompressionProvider provider,
    ...
    private IAdaptiveCompressionProvider? InitializeCompressionHeaders()
    {
        if (_provider.ShouldCompressResponse(_context))
        {
            ...
            {
                headers[HeaderNames.ContentEncoding] = StringValues.Concat(headers[HeaderNames.ContentEncoding], compressionProvider.EncodingName);
    ...
    private IAdaptiveCompressionProvider? ResolveCompressionProvider()
    {
        if (!_providerCreated)
        {
            _providerCreated = true;
            _compressionProvider = _provider.GetAdaptiveCompressionProvider(_context);
        }
